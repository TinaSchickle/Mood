    ...
    <div className="max-w-xl mx-auto px-4 pb-28">
      ...
      <div className="flex items-center justify-between gap-2 py-3 mb-1">
        <button
          ...
          className="w-10 h-10 rounded-full card grid place-items-center text-xl text-indigo-500 active:scale-90 transition"
        ...
        <div className="text-center">
          ...
          <label className="text-xs font-semibold cursor-pointer" style={{ color: 'var(--ink-soft)' }}>
            ...
            <input
              ...
              className="block mx-auto mt-0.5 bg-transparent text-center outline-none text-indigo-400"
            />
          </label>
        </div>
        <button
          onClick={() => setDate(addDays(date, 1))}
          disabled={isToday}
          className="w-10 h-10 rounded-full card grid place-items-center text-xl text-indigo-500 active:scale-90 transition disabled:opacity-30"
          aria-label="Next day"
        >
          ›
        </button>
      ...
      <section className="card p-4 mb-3">
        <div className="flex items-center gap-2 mb-2">
          ...
          {day != null && (
            <span className="ml-auto text-sm font-bold text-rose-600 bg-rose-50 rounded-full px-3 py-1">
              Cycle day {day}
            </span>
          )}
        </div>
